...
import { createTrail } from "../graphql/mutations";
const client = generateClient();
export default function TrailCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setDistanceMiles(initialValues.distanceMiles);
    setElevationGain(initialValues.elevationGain);
    setImageUrl(initialValues.imageUrl);
    setLocation(initialValues.location);
    setTrailPath(initialValues.trailPath);
    setErrors({});
  };
  const validations = {
    name: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createTrail.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "TrailCreateForm")}
      {...rest}
    >
      <TextField
        label="Name"
